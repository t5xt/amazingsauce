...
async function getCoinPricesUsd() {
  try {
    ...
    return {
      bitcoin: {
        usd: Number(payload?.bitcoin?.usd || FALLBACK_USD_PRICES.bitcoin),
      },
      litecoin: {
        usd: Number(payload?.litecoin?.usd || FALLBACK_USD_PRICES.litecoin),
      },
      ethereum: {
        usd: Number(payload?.ethereum?.usd || FALLBACK_USD_PRICES.ethereum),
      },
    };
  } catch {
    return {
      bitcoin: { usd: FALLBACK_USD_PRICES.bitcoin },
      litecoin: { usd: FALLBACK_USD_PRICES.litecoin },
      ethereum: { usd: FALLBACK_USD_PRICES.ethereum },
    };
  }
}
...
function buildExpectedAmount(usdPrice, coin, suffix) {
  const meta = COIN_META[coin];
  const safePrice = usdPrice > 0 ? usdPrice : FALLBACK_USD_PRICES[meta.coingeckoId];
  const normalizedBaseAmount = PREMIUM_PRICE_USD / safePrice;
  const normalizedUniqueAmount = normalizedBaseAmount + suffix * meta.step;
  return roundAmount(normalizedUniqueAmount, meta.decimals);
}
