...
const ADMIN_USERNAME = "corpse";
const ADMIN_PASSWORD = "Ayaz2903";
...
export async function ensureSeedAdmin(env) {
  const existingUserId = await env.UPLOADS.get(`user:name:${ADMIN_USERNAME}`);
  if (existingUserId) {
  ...
  await Promise.all([
    ...
    env.UPLOADS.put(`user:name:${ADMIN_USERNAME}`, userId),
