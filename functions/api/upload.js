...
const ID_LENGTH = 12;
const FREE_ALLOWED_HOURS = new Set([3, 24, 48]);
...
const PREMIUM_MAX_DURATION_HOURS = 24 * 365;
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const maxFileSizeBytes = isPremium ? PREMIUM_MAX_FILE_SIZE_BYTES : FREE_MAX_FILE_SIZE_BYTES;

    if (!file || typeof file.arrayBuffer !== "function") {
      return json({ error: "Please upload a media file." }, { status: 400 });
    }

    if (!String(file.type || "").match(/^(image|video)\//)) {
      return json({ error: "Only image and video uploads are allowed." }, { status: 400 });
    }
    ...
    if (!isPremium && !FREE_ALLOWED_HOURS.has(expiresHours)) {
      return json({ error: "Free uploads only support 3h, 24h, or 48h expiry." }, { status: 400 });
    }

    if (isPremium && expiresHours > PREMIUM_MAX_DURATION_HOURS) {
      return json({ error: "Premium custom duration is capped at 8760 hours." }, { status: 400 });
    }
    ...
    if (Number(file.size || 0) > maxFileSizeBytes) {
      ...
      return json({
        error: isPremium
          ? `Premium uploads are limited to ${maxMb} MB.`
          : `Free uploads are limited to ${maxMb} MB. Upgrade to premium for 3x bigger uploads.`,
      }, { status: 400 });
    }
